// app.js – patched to auto-add Import link (no async/await), VER=21
const VER = '21';
const app = document.getElementById('app');
const nav = document.getElementById('nav');
const statusEl = document.getElementById('sb-status');

// Ensure "Import" link exists so you don't have to edit HTML
(function ensureImportLink(){
  if (!nav) return;
  if (!nav.querySelector('[data-page="import"]')){
    var a = document.createElement('a');
    a.setAttribute('href', '#');
    a.setAttribute('data-page', 'import');
...
document.getElementById('btn-refresh').addEventListener('click', function () {
  ...
  var active = activeEl ? activeEl.getAttribute('data-page') : 'crm';
  loadPage(active);
});
...
function importFallback(page){
  ...
  function next(i){
    ...
    return import(url).catch(function(err){ console.error('[loader] import failed', url, err); last = err; return next(i+1); });
  }
  return next(0);
}
