...
async function loadPage(page) {
  ...
  try {
    const modUrl = `./pages/${page}.js?v=11`;
    const module = await import(modUrl);
    await module.default(app);
  } catch (err) {
    console.error('Module load error:', err);
    app.innerHTML = `<div class="alert">Module <strong>${page}</strong> niet gevonden of met fout geladen.<br><small>Probeerde: <code>${page}.js?v=11</code></small></div>`;
  }
}
