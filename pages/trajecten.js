// Trajecten - volledige module (conservatief, zonder async/await / optional chaining)
...
function nlFromISO(v){
  if(!v) return '';
  var s = String(v).slice(0,10).split('-');
  return (s.length===3) ? (s[2] + '-' + s[1] + '-' + s[0]) : v;
}

var state = { list: [], editId:null, club:null, openAfterLoad:null };
...
`<div class="panel">
  <div class="toolbar" style="display:flex;gap:8px;align-items:center">
    ...
    <div style="margin-left:auto;display:flex;gap:8px;align-items:center">
      <label class="mute" for="f-status-filter" style="font-size:12px">Status</label>
...
<div id="modal" class="panel" style="display:none;position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);max-height:85vh;overflow:auto;width:880px;z-index:1000">
  ...
  <div class="grid" style="grid-template-columns:1fr 1fr;gap:12px;margin-top:12px">
    <div>
      ...
      <input id="club-q" type="text" placeholder="Zoek/Selecteer club…" />
    ...
    <div>
      ...
      <input id="f-start" placeholder="dd-mm-jjjj" />
    </div>
    <div>
      <label>Verwacht einde</label>
      <input id="f-eind" placeholder="dd-mm-jjjj" />
    </div>

    <div>
      <label>Begroot (€)</label>
      <input id="f-begroot" value="0,00" />
    </div>
    <div>
    ...
    <div>
      ...
      <input id="f-fin-pct" placeholder="%" />
    </div>
    <div>
      <label>Financiering €</label>
      <input id="f-fin-eur" value="0,00" />
    </div>

    <div>
      <label>Eigen bijdrage %</label>
      <input id="f-eigen-pct" placeholder="%" />
    </div>
    <div>
      <label>Eigen bijdrage €</label>
      <input id="f-eigen-eur" value="0,00" />
    </div>
...
function bindModal(){
  ...
  function close(){ overlay.style.display='none'; modal.style.display='none'; state.editId=null; state.club=null; $('#dlg-title').textContent='Nieuw traject'; }
...
function renderList(){
  var list = $('#list'), status = $('#f-status-filter').value;
  var items = (state.list||[]).filter(function(r){ return !status || r.status===status; });
  if (!items.length){ list.innerHTML = '<div class="mute...">Geen trajecten gevonden.</div>'; return; }
  list.innerHTML = items.map(function(r){
    return (
      ...
      '<div style="margin-top:6px;font-size:12px">Begroot: € ' + fmtEur(r.begroot_eur) + '</div>' +
...
function loadList(done){
  ...
  supabase
    ...
    .then(function({ data, error }){
      if (error){ console.error(error); $('#list').innerHTML = '<div class="alert err">Laden mislukt</div>'; if(done) done(error); return; }
      state.list = data || [];
      renderList();
      if (done) done(null, state.list);
    });
}
...
function collect(){
  ...
  var club = state.club || {};
  return {
    club_nr: club['Nr.'] ? String(club['Nr.']) : null,
    club_naam: club['Naam'] || null,
    gemeente: club['Vestigingsgemeente'] || null,

    titel: club['Naam'] || null,
    type: $('#f-type').value,
    status: $('#f-status').value,
    eigenaar: $('#f-eigenaar').value,
    notities: $('#f-note').value || null,

    start_datum: isoFromNL($('#f-start').value),
    eind_datum: isoFromNL($('#f-eind').value),
    laatste_update: null,

    begroot_eur: begroot || 0,
    financiering_type: $('#f-fin-type').value,
    financiering_pct: finPct,
    financiering_eur: finEur || 0,
    eigen_pct: eigPct,
    eigen_eur: eigEur || 0
  };
}
...
function bindFinance(){
  ...
  function recalcFromPct(){
    ...
    var b = parseEur($('#f-begroot').value);
    var p = parsePct($('#f-fin-pct').value);
    var ep = parsePct($('#f-eigen-pct').value);
  ...
  function recalcFromEur(){
    if (lock) return; lock=true;
    var b = parseEur($('#f-begroot').value);
    var e = parseEur($('#f-fin-eur').value);
    var ee = parseEur($('#f-eigen-eur').value);
    if (b>0 && e){ $('#f-fin-pct').value = String((100*e/b).toFixed(2)).replace('.',','); }
    if (b>0 && ee){ $('#f-eigen-pct').value = String((100*ee/b).toFixed(2)).replace('.',','); }
    lock=false;
  }
  ['f-begroot','f-fin-pct','f-eigen-pct'].forEach(function(id){ on($('#'+id),'input',recalcFromPct); });
  ['f-fin-eur','f-eigen-eur'].forEach(function(id){ on($('#'+id),'input',recalcFromEur); });
}
...
function bindClubSearch(){
  ...
  function search(q){
    ...
    supabase
      .from('clubs')
      .select('"Nr.", Naam, "Vestigingsgemeente"')
      .or(expr)
      .limit(10)
      .then(function({data, error}){
...
export default function mount(app){
  ...
  on($('#btn-new'),'click',function(){
    ...
    $('#club-q').value=''; $('#f-note').value='';
    ['f-start','f-eind','f-begroot','f-fin-pct','f-fin-eur','f-eigen-pct','f-eigen-eur'].forEach(function(id){ var el=$('#'+id); if(el) el.value=''; });
  ...
  document.addEventListener('click',function(e){
    ...
    if (r){ state.editId=r.id; prefill(r); modal.open(); }
  });
  ...
  on($('#dlg-save'),'click',function(){
    ...
    var req = state.editId
      ? supabase.from('trajecten').update(payload).eq('id', state.editId)
    ...
    req.then(function({ error }){
      ...
      modal.close();
      loadList(function(){
        // Als CRM een open verzoek had gezet:
        var openId = sessionStorage.getItem('openTrajectId');
        if (openId){ sessionStorage.removeItem('openTrajectId'); }
      });
    });
  });

  // Init load + deep-link vanuit CRM
  var toOpen = sessionStorage.getItem('openTrajectId');
  loadList(function(err, rows){
    if (!err && toOpen){
      var rec = (rows||[]).find(function(x){ return String(x.id)===String(toOpen); });
      if (rec){ state.editId=rec.id; prefill(rec); modal.open(); sessionStorage.removeItem('openTrajectId'); }
    }
  });
}